...
function rebuildString(target_array, count) {
    ...
    if (!target_array) throw "No array provided";
...
function removeArrayLabels(args) {
    ...
    for (i = 0; i < args.length; i++) {
        ...
        if (args[i] === "[") t_in_array.push(1);

        if (args[i] === "{") t_in_array.push(0);

        if (args[i] === "]" || args[i] === "}") t_in_array.pop();
...
function commandParam(name, required, key, has_arg, arg_type, prefix) {
    ...
    commandParam.prototype.check = function (value) {
        /* Is a value required ? */
        if (!value || value === "") {
            if ((this.arg_type === "keyAndArg") || (this.arg_type === "argOnly")) {
                throw "Error you must provide a value to arg " + this.name;
            }
        ...
        if (!this.arg_type.length) return;
...
function command(name, action_name, params) {
    ...
    command.prototype.generate = function (args) {
        if (typeof (args) !== "object") throw "Wrong argument type";
...
function commandList() {
    ...
    commandList.prototype.insert = function (command) {
        ...
        if (this.commandList[ command.name ]) {
            throw "Pushing command " + command.name + " would shadow a previous registration";
        }
    ...
    commandList.prototype.dumpCommandList = function () {
        ...
        for (var e in this.commandList) {
            ...
            for (j = 0; j < c.params.length; j++) {
                ...
                if ((a.has_arg === "argOnly") || (a.has_arg === "keyAndArg")) {
...
function gdbMI(command_and_args, options, gdbWrapper) {
    ...
    gdbMI.prototype.getOpt = function (name) {
        if (!this.opt) return undefined;

        if (!this.opt[ name ]) return undefined;
